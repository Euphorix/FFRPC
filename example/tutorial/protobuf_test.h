...
struct protobuf_test_t
{
    ...
    void echo_callback(ffreq_t<pb_echo_out_t>& req_, int index, ffrpc_t* ffrpc_client)
    {
        ...
        else if (index < 10)
        {
            ...
            sleep(1);
            ffrpc_client->call("echo", in, ffrpc_ops_t::gen_callback(&protobuf_test_t::echo_callback, this, ++index, ffrpc_client));
        }
        else
        {
...
static  int run_protobuf_test(arg_helper_t& arg_helper)
{
    ...
    ffrpc_client.call("echo", in, ffrpc_ops_t::gen_callback(&protobuf_test_t::echo_callback, &foo, 1, &ffrpc_client));
